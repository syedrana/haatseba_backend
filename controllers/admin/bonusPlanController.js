...
const createBonusPlan = async (req, res) => {
  try {
    const { level, bonusAmount, costValue, rewardType, condition, status } = req.body;
    ...
    const newBonus = new BonusPlan({ level, bonusAmount, costValue, rewardType, condition, status });
    await newBonus.save();
...
const updateBonusPlan = async (req, res) => {
  try {
    ...
    if (!updated) return res.status(404).json({ message: "Bonus plan not found" });
